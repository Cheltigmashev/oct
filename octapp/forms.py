...
class ClosedQuestionOptionForm(forms.ModelForm):
    add_several = forms.BooleanField(
        ...
        required=False,
        help_text=(u'Для разделения вариантов ответа используйте переносы строк (enter). Кнопка «Источник» должна быть при этом неактивной.')
    )
...
class SequenceQuestionElementForm(forms.ModelForm):
    add_several = forms.BooleanField(
        label=(u'Добавить несколько'),
        required=False,
        help_text=(u'Для разделения элементов последовательности используйте переносы строк (enter). Кнопка «Источник» должна быть при этом неактивной.')
    )
...
class ComparisonQuestionElementForm(forms.ModelForm):
    add_several = forms.BooleanField(
        label=(u'Добавить несколько'),
        required=False,
        help_text=(u'Для разделения элементов левого или правого ряда сопоставления используйте переносы строк (enter). Кнопка «Источник» должна быть при этом неактивной.')
    )
