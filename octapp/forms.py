...
class TestForm(forms.ModelForm):
    new_category = forms.CharField(
        ...
        widget=TextInput(attrs={'title': 'Допускается использовать строчные буквы, цифры, дефис и /', 'placeholder': 'Допускается использовать строчные буквы, цифры, дефис и /', 'pattern': '[ a-zа-я0-9 -/]+'}),
    ...
    new_tags = forms.CharField(
        ...
        required=False,
        widget=TextInput(attrs={'title': 'Допускается использовать строчные буквы, цифры, запятые, дефис и /', 'placeholder': 'Допускается использовать строчные буквы, цифры, запятые, дефис и /', 'pattern': '[ a-zа-я0-9 -/,]+'}),
    ...
    class Meta:
        ...
        widgets = {
            ...
            'name': TextInput(attrs={'title': 'Первая буква названия будет преобразована в заглавную, остальные — в строчные. Допускается использовать: буквы, цифры, пробелы, запятые, -/«»():;', 'placeholder': 'Допускается использовать: буквы, цифры, пробелы, запятые, -/«»():;', 'pattern': '[a-zA-Zа-яА-Я0-9 -/,«»();:]*'}),
        }
        error_messages = {
            'name': {
