...
class ComparisonQuestionForm(forms.ModelForm):
    ...
    add_comp_elements = forms.BooleanField(
        ...
        required=False,
        help_text=(u'Введите после содержимого самого вопроса (с новой строки, используя enter) слово «ЛЕВЫЙ_РЯД», затем вновь перенос строки, а затем один или более элементов ЛЕВОГО ряда сопоставления, разделяя их переносами строк (используйте enter, кнопка «Источник» должна быть неактивной. Аналогично можете ввести элементы ПРАВОГО ряда, используя строку-разделитель «ПРАВЫЙ_РЯД». Минимум 2 варианта/элемента.')
    )
